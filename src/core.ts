...
const RTL_STRONG_G =
  /[\u0591-\u08ff\ufb1d-\ufdff\ufe70-\ufeff]/gu
const LTR_STRONG =
  /[A-Za-z\u00c0-\u02af\u0370-\u052f\u10a0-\u10ff\u1e00-\u1eff]/u
const LTR_STRONG_G =
  /[A-Za-z\u00c0-\u02af\u0370-\u052f\u10a0-\u10ff\u1e00-\u1eff]/gu
const DIRECTIONAL_CONTROLS =
  /[\u061c\u200e\u200f\u202a-\u202e\u2066-\u2069]/gu
const LATIN_TOKEN =
  /[A-Za-z0-9][A-Za-z0-9._~:/#%+@*-]*/g
const RTL_TOKEN =
  /[\u0591-\u08ff\ufb1d-\ufdff\ufe70-\ufeff][\u0591-\u08ff\ufb1d-\ufdff\ufe70-\ufeff\u200c\u200d\u0660-\u0669\u06f0-\u06f9.,\-_]*/gu
...
export function normalizeOptions(input: unknown): NormalizedRtlOptions {
  ...
  return {
    ...
    minRtlCharacters: Math.max(
      0,
      Math.trunc(readNumber(raw.minRtlCharacters, DEFAULT_OPTIONS.minRtlCharacters, 0, 1000)),
    ),
...
export function analyzeDirection(
  text: string,
  options: Pick<NormalizedRtlOptions, "language" | "minRtlCharacters" | "minRtlRatio"> = DEFAULT_OPTIONS,
): DirectionAnalysis {
...
export function resolveDirection(
  text: string,
  options: Pick<NormalizedRtlOptions, "language" | "minRtlCharacters" | "minRtlRatio" | "forceDirection">,
): Direction {
  if (options.forceDirection !== "auto") return options.forceDirection
...
function isolateLines(text: string, mode: IsolationMode, options: NormalizedRtlOptions): string {
...
function alignAndIsolate(text: string, mode: IsolationMode, options: NormalizedRtlOptions): string {
  const { lines, newlines } = splitKeepNewlines(text)
...
function formatBlockLine(line: string, mode: IsolationMode, options: NormalizedRtlOptions): string {
  ...
  if (!shouldIsolate) return `${marker}${withDigits}`
...
function formatTableRow(line: string, mode: IsolationMode, options: NormalizedRtlOptions): string {
  const leading = line.match(/^\s*/u)?.[0] ?? ""
  const trimmed = line.trim()
  const cells = trimmed.slice(1, -1).split("|")
  const formatted = cells.map((cell) => {
    if (isTableSeparatorCell(cell)) return cell
    ...
    const shouldIsolate = mode === "always" || (mode === "auto" && direction === "rtl")
    if (!shouldIsolate) return `${leadingCell}${applyDigitMode(content, options.digitMode)}${trailingCell}`
...
function isolateWithProtection(text: string, direction: Direction, options: NormalizedRtlOptions): string {
...
function protectRuns(text: string, direction: Direction): string {
  if (direction === "neutral") return text
  if (!containsOpposite(text, direction)) return text
...
function containsOpposite(text: string, direction: Direction): boolean {
  const other = direction === "rtl" ? LTR_STRONG : RTL_STRONG
  other.lastIndex = 0
  return other.test(text)
}

function protectPlain(text: string, direction: Direction): string {
  if (direction === "rtl") {
...
function splitInlineCode(text: string): Array<{ code: boolean; delimiter: string; text: string }> {
  const segments: Array<{ code: boolean; delimiter: string; text: string }> = []
  let lastIndex = 0
...
function isolate(text: string, direction: Direction) {
  const start = direction === "rtl" ? RLI : LRI
  return `${start}${text}${PDI}`
}

function wrapVisualText(text: string, column: number): string[] {
...
function splitLongWord(word: string, column: number): string[] {
...
function findFirstStrong(text: string): Direction {
...
function splitKeepNewlines(text: string): { lines: string[]; newlines: string[] } {
...
function joinLines(lines: string[], newlines: string[]): string {
  let result = ""
  for (let i = 0; i < lines.length; i++) {
...
function readSystemGuidance(value: unknown, fallback: boolean | string): boolean | string {
